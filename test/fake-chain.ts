...
import { BondingCurveAbi, CreatorBondAbi, FloorReserveAbi, LPLockerAbi, MAINNET, PinarcFactoryAbi, PinarcTokenAbi, VestingVaultAbi } from "@pinarc-labs/sdk";
import type { ChainClient, CurveParams, Readers, TokenInfo } from "../src/index.js";

type AbiLike = readonly unknown[];
const ABIS: Record<string, AbiLike> = { factory: PinarcFactoryAbi, curve: BondingCurveAbi, bond: CreatorBondAbi, floor: FloorReserveAbi, locker: LPLockerAbi, vault: VestingVaultAbi, token: PinarcTokenAbi };
...
export class FakeChain implements ChainClient {
  ...
  emit(block: bigint, source: keyof typeof ABIS, address: Address, eventName: string, args: Record<string, unknown>): Log {
    ...
    const log = { address, blockNumber: block, blockHash: this.hashOf(block), transactionHash: keccak256(toHex(`tx-${block}-${this.nonce}`)), transactionIndex: 0, logIndex: this.nonce++, removed: false, data, topics } as unknown as Log;
    this.logs.push(log);
